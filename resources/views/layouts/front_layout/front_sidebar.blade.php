...
<div id="sidebar" class="span3">
    <div class="well well-small"><a id="myCart" href="{{ url('/cart') }}"><img src="{{ asset('images/front_images/ico-cart.png') }}" alt="cart"><span class="totalCartItems">{{ totalCartItems() }}</span> produits</a></div>
    ...
    @if (isset($page_name) && $page_name=="listing")
        ...
        <div class="well well-small">
            ...
            @foreach ($heightArray as $height)
                <input class="height" style="margin-top: -3px" type="checkbox" name="height[]" id="{{ $height }}" value="{{ $height }}"> &nbsp;&nbsp;{{ $height }}<br>
            @endforeach
        </div>
        <div class="well well-small">
            <h5>Motif</h5>
